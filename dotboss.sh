...
start_gitwatch() {
	...
	nohup gitwatch -r "${DOT_REPO_REMOTE}" -b "${DOT_REPO_BRANCH}" -m "Auto sync dotfiles on change (%d) by dotboss" "${DOT_REPO_DIR}" >"$HOME"/.dotboss_log/watch.log &
	...
	sudo echo "su ${USER} -c \"/usr/local/bin/gitwatch -r ""${DOT_REPO_REMOTE}"" -b ""${DOT_REPO_BRANCH}"" -l ""${USER}"" -m ""Auto sync dotfiles on change (%d) by dotboss"" ""${DOT_REPO_DIR}"" > ""$HOME""/.dotboss_log/watch.log &""\"" | sudo tee /etc/rc.local
}
